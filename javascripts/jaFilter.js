    ...
    var namespace = '.jaFilter',
        ...
        filters = {
            ...
            setAttributeCount: function(categoriesCnt, action) {
                ...
                options.$selector.find('[type="checkbox"]').each(function() {
                    ...
                    /*Add filter counter*/
                    if ((action != 'init' && options.neverRecount != null && $.inArray(attr, options.neverRecount) != -1) || (options.categoryRecount === true && options.onClickCategory == attr)) {
                        // do nothing??
                    } else {
                        var value = $(this).val();
                        var cnt;
                        ...
                        if (typeof $labelFor === "undefined") { // Add counter in next tag
                            $(this).next().text(cnt);
                        } else { // Add the counter in label based on input id
                            $labelFor.find('span').text(cnt);
                        }

                        // Hide filters that has 0 counted elements
                        if ($.isArray(options.noEmptyFilters) && $.inArray(attr, options.noEmptyFilters) != -1 && cnt == 0 || options.noEmptyFilters == false) {
                            $(this).parent().hide();
                            $labelFor.hide();
                            $labelFor.next().hide();
                        } else { // or show them
        ...
        methods = {
            ...
            getCache: function(resetCache) {
                if (resetCache || typeof resetCache === "undefined") methods.resetCache();
